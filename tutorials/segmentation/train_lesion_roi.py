...
RESULT = "./result_lesion_roi_focal_a05_g2"
...
def dice_loss(logits, target, eps=1e-6):
    ...
    Expects raw logits from model (NO sigmoid in UNet forward).
    """
    ...
    # ---- FOCAL LOSS (binary) ----
    # parameters to tune:
    #   alpha: balances positives vs negatives (0.25 common)
    #   gamma: focuses on hard examples (2.0 common)
    alpha = 0.5
    gamma = 2.0
    ...
    while step < total_steps:
        for img_t, lab_t, _, _ in train_loader:
            ...
            if step % test_every == 0 or step == total_steps:
                img_t2, _, img_name2, _ = next(iter(test_loader))
                img_name2 = img_name2[0]
                ...
                save_path = os.path.join(RESULT, f"pred_step{step:04d}_id000.npy")
