...
# Settings
# -------------------------------
FOLDER = "./data/lesion-data"
RESULT = "./result_lesion"
...
class NPYDataset(torch.utils.data.Dataset):
    def __init__(self, folder, split):
        ...
        self.split = split
        self.files = sorted(
            [f for f in os.listdir(folder) if f.startswith(f"image_{split}")]
        )
...
if __name__ == "__main__":
    ...
    if len(train_ds) == 0:
        raise RuntimeError("❌ No train files found — run prepare_prostate_data.py first.")
    if len(test_ds) == 0:
